...
class dreem_model:
    def __init__(self, settings):
        ...
        # prepare normalization for eeg / resp data
        ...
        # extract relevant data into appropriate subgroups: xxx_data_{eeg / resp / meta}_{x / y}
        ...
        # incorporate OHE in the self.xxx_meta_data
        ...
        print(np.shape(self.train_data_y))
    ...
    def apply_model(self):
        ...
        # option 3: CNN then gboost on top of the activations
    ...
    def convolution_model(self):
        ...
        # best optimizer is adam, leave settings to default
        optimizer = 'adam'
        ...
        if self.settings['display']:
            # summarize history for loss => draw curves
            plt.plot(history.history['loss'])
            plt.plot(history.history['val_loss'])
            plt.title('Evolution of training error and validation error during training')
            plt.ylabel('Loss')
            plt.xlabel('Number of epochs')
            plt.axis([0, 8, 0.15, 0.45])
            plt.legend(['training error', 'validation error'], loc='upper right')
            plt.show()

        # print summary of what the NN looks like
    ...
    def simple_gradient_boost_model(self, train_data_x, train_data_y, valid_data_x, valid_data_y, test_data_x):
        ...
        if self.settings['lightgbm']:
            if self.settings['grid_search']:
                ...
                models = lgb.LGBMRegressor(objective='regression', reg_alpha=100, reg_lambda=100, n_estimators=1000)
